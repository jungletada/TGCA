...
if [[ "$(git branch --show-current)" != main ]]; then
    echo "The Class-Stable LaST experiment must start from main." >&2
    exit 2
fi
...
tgca_run_id=${TGCA_RUN_ID:-"$(date +%Y%m%d)-class-stable-last-voc-s0-${tgca_short_commit}"}
tgca_run_root="$tgca_repo_root/results/class_stable_last/$tgca_run_id"
...
tgca_last_root=${TGCA_LAST_MCT_SOURCE_RUN:-"$tgca_repo_root/results/last_mct/20260906-last-mct-voc-s0-310fd1d"}
...
tgca_last_summary_sha=${TGCA_LAST_MCT_SOURCE_SUMMARY_SHA:-683c13a493bc4ecb6e0c21750787ee5a1cc76f83859d8ed6ce22b8d8c313646b}
...
for tgca_path in \
        ...
        "$tgca_last_root/PIPELINE_COMPLETE" \
        "$tgca_last_root/EXPERIMENT_COMPLETE" \
...
if [[ "$(sha256sum "$tgca_last_root/summary.json" | awk '{print $1}')" != "$tgca_last_summary_sha" ]]; then
    echo "Previous Last-MCT source summary hash mismatch." >&2
    exit 2
fi
...
printf 'bash experiments/ablations/run_class_stable_last_voc.sh\n' \
...
value = {
    'experiment_name': 'Class-Stable LaST Pooling',
    ...
    'topk_dimension': 'spatial, independently per semantic class',
    'selected_value_source': 'original M',
    'lowpass_role': 'selector only; no auxiliary loss',
    'patch_head': 'shared Conv2d(384,20,kernel_size=3,padding=1)',
    'cam_source': 'original M',
...
python - "$tgca_source_root" "$tgca_patch_root" "$tgca_last_root" \
        "$tgca_source_summary_sha" "$tgca_patch_summary_sha" \
        "$tgca_last_summary_sha" "$tgca_run_root/source_reference.json" <<'PY'
import hashlib, json, pathlib, sys
source, patch, last = map(pathlib.Path, sys.argv[1:4])
...
def sha(path):
    ...
    with path.open('rb') as stream:
        for chunk in iter(lambda: stream.read(1024 * 1024), b''):
            h.update(chunk)
    return h.hexdigest()
summaries = [json.loads((root / 'summary.json').read_text()) for root in (source, patch, last)]
for root, summary, digest in zip((source, patch, last), summaries, expected):
    if summary.get('status') != 'complete' or sha(root / 'summary.json') != digest:
        raise SystemExit(f'immutable matched source validation failed: {root}')
files = {
    ...
    'last_summary': last / 'summary.json',
    'last_classification': last / 'evaluations/last_mct/classification/classification_metrics.json',
    'last_cam': last / 'evaluations/last_mct/cam_evaluation/metrics.json',
    'last_audit': last / 'audit/last_mct.json',
}
checkpoints = {
    'original': summaries[0]['source_checkpoints']['baseline'],
    'patch_final_ln': summaries[1]['patch_checkpoint'],
    'last_mct': summaries[2]['last_checkpoint'],
}
...
run_exact_logged "$tgca_run_root/tests.txt" python -m pytest -q \
    tests/test_mctformerplus_class_stable_last.py \
...
printf 'STAGE=tests finished=%s\n' "$(date --iso-8601=seconds)" \
    | tee -a "$tgca_run_root/tests.txt" "$tgca_run_root/training_logs/tests_complete.txt"
...
run_exact_logged "$tgca_smoke_train_log" \
    ...
    --patch-final-norm --class-stable-last
...
run_exact env CUDA_VISIBLE_DEVICES="$tgca_gpu" python \
    ...
    --output "$tgca_run_root/smoke/audit/class_stable_last.json" \
    --patch-final-norm --class-stable-last
run_exact env CUDA_VISIBLE_DEVICES="$tgca_gpu" python make_cam.py \
    ...
    --patch-final-norm --class-stable-last
python - "$tgca_smoke_train_log" \
        "$tgca_run_root/smoke/audit/class_stable_last.json" \
        "$tgca_run_root/smoke/cam" \
        "$tgca_run_root/smoke/smoke_summary.json" <<'PY'
...
tgca_train_root="$tgca_run_root/checkpoints/class_stable_last"
tgca_eval_root="$tgca_run_root/evaluations/class_stable_last"
printf 'STAGE=class_stable_train started=%s\n' "$(date --iso-8601=seconds)"
...
cp --no-clobber "$tgca_source_log" \
    "$tgca_run_root/training_logs/class_stable_last.log"
printf 'STAGE=class_stable_train finished=%s\n' "$(date --iso-8601=seconds)"

printf 'STAGE=class_stable_audit started=%s\n' "$(date --iso-8601=seconds)"
run_exact env CUDA_VISIBLE_DEVICES="$tgca_gpu" python \
    tools/audit_mctformerplus_variant.py \
    ...
    --output "$tgca_run_root/audit/class_stable_last.json" \
    --patch-final-norm --class-stable-last

printf 'STAGE=class_stable_classification started=%s\n' "$(date --iso-8601=seconds)"
run_exact env CUDA_VISIBLE_DEVICES="$tgca_gpu" python \
...
printf 'STAGE=class_stable_cam_train started=%s\n' "$(date --iso-8601=seconds)"
run_exact env CUDA_VISIBLE_DEVICES="$tgca_gpu" python make_cam.py \
    --dataset VOC12 --model mctformerplus \
...
printf 'STAGE=class_stable_cam_evaluation started=%s\n' "$(date --iso-8601=seconds)"
