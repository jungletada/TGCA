...
if [[ "$(git branch --show-current)" != main ]]; then
    echo "The Residual-CWP experiment must start from main." >&2
    exit 2
fi
...
tgca_seed=0
tgca_epochs=(25 30 35 45)
tgca_commit=$(git rev-parse HEAD)
tgca_short_commit=$(git rev-parse --short HEAD)
tgca_run_id=${TGCA_RUN_ID:-"$(date +%Y%m%d)-mctformerplus-residual-cwp-voc-s0-${tgca_short_commit}"}
tgca_run_root="$tgca_repo_root/results/residual_cwp/$tgca_run_id"
...
mkdir -p "$tgca_run_root"/{smoke,training_logs}
...
printf 'bash experiments/ablations/run_mctformerplus_residual_cwp_voc.sh\n' \
...
value = {
    'experiment_name': 'Residual Image-Conditioned Multi-Class Token Initialization',
    'model': 'mctformerplus', 'variant': 'small',
    'dataset': 'PASCAL VOC 2012', 'seed': 0, 'input_size': 448,
    'independent_epoch_schedules': [25, 30, 35, 45],
    ...
    'class_token_init': 'residual_cwp', 'query_shape': [20, 384],
    'query_initialization': 'trunc_normal_std_0.02',
    'alpha_initialization': 0.1, 'alpha': 'single_learnable_scalar',
    'initial_tokens': 'repeated_DeiT_CLS + alpha * softmax(Q@P.T/sqrt(D))@P + pos_embed_cls',
...
run_exact_logged "$tgca_run_root/tests.txt" python -m pytest -q \
    tests/test_mctformerplus_residual_cwp.py \
...
run_exact_logged "$tgca_smoke_log" \
    ...
    --dataset VOC12 --model mctformerplus --class-token-init residual_cwp \
...
run_exact env CUDA_VISIBLE_DEVICES="$tgca_gpu" python tools/audit_mctformerplus_variant.py \
    ...
    --class-token-init residual_cwp --official-pretrained "$tgca_pretrained" \
    --expected-pretrained-sha256 "$tgca_pretrained_sha" \
    --expected-epochs 1 --expected-effective-batch 32 --expected-seed 0 \
    --output "$tgca_run_root/smoke/audit/residual_cwp.json"
...
python - "$tgca_smoke_log" "$tgca_run_root/smoke/audit/residual_cwp.json" \
...
required = {
    'loss': values('loss'), 'alpha': values('residual_cwp_alpha'),
    'entropy': values('cwp_entropy_mean'),
    'similarity': values('cwp_attention_interclass_cosine'),
...
if not all(math.isfinite(value) for seq in required.values() for value in seq):
    raise SystemExit('non-finite smoke diagnostic')
audit = json.loads(audit_path.read_text())
cams = sorted(cam_dir.glob('*.npy'))
payload = {
    'status': 'pass', 'training_iterations': 100,
    'alpha_initial': 0.1, 'alpha_observed_final': required['alpha'][-1],
...
if (not payload['strict_audit_passed'] or not payload['cam_complete']
        or len(cams) != 4 or payload['attention_row_sum_max_error'] >= 1e-6):
...
for tgca_epoch_count in "${tgca_epochs[@]}"; do
    tgca_epoch_root="$tgca_run_root/epochs_${tgca_epoch_count}"
    tgca_train_root="$tgca_epoch_root/checkpoints/residual_cwp"
    tgca_eval_root="$tgca_epoch_root/evaluations/residual_cwp"
    mkdir -p "$tgca_epoch_root"/{audit,checkpoints,evaluations,training_logs}
    printf 'STAGE=residual_cwp_%s_train started=%s\n' \
        "$tgca_epoch_count" "$(date --iso-8601=seconds)"
    run_exact_logged "$tgca_epoch_root/training_stdout.log" \
        env CUDA_VISIBLE_DEVICES="$tgca_gpu" python train_model_v2.py \
        --dataset VOC12 --model mctformerplus --class-token-init residual_cwp \
        --voc12_root "$tgca_voc_root" \
        --train_list "$tgca_train_aug" --val_list "$tgca_val" \
        --work_space "$tgca_train_root" --input-size 448 \
        --epochs "$tgca_epoch_count" --batch_size 32 --accum-iter 1 \
        --val-batch-size 32 --seed "$tgca_seed" --opt adamw --sched cosine \
        --warmup-epochs 5 --lr 5e-4 --min-lr 1e-5 --weight-decay 0.05 \
        --drop 0.0 --drop-path 0.1 --train-interpolation bicubic \
        --attention-normalization vanilla --attention-gamma 1.0 \
        --bcss-variant e0 --psl-variant baseline \
        --finetune "$tgca_pretrained" --num_workers 10
    tgca_checkpoint="$tgca_train_root/mctformerplus_final.pth"
    sha256sum "$tgca_checkpoint" > "$tgca_epoch_root/checkpoint_manifest.txt"
    tgca_source_log=$(find "$tgca_train_root/log_dir" -maxdepth 1 \
        -type f -name 'train-*.log' -print -quit)
    cp --no-clobber "$tgca_source_log" \
        "$tgca_epoch_root/training_logs/residual_cwp.log"
    printf 'STAGE=residual_cwp_%s_train finished=%s\n' \
        "$tgca_epoch_count" "$(date --iso-8601=seconds)"
    ...
    run_exact env CUDA_VISIBLE_DEVICES="$tgca_gpu" python tools/audit_mctformerplus_variant.py \
        ...
        --class-token-init residual_cwp --official-pretrained "$tgca_pretrained" \
        --expected-pretrained-sha256 "$tgca_pretrained_sha" \
        --expected-epochs "$tgca_epoch_count" --expected-effective-batch 32 \
        --expected-seed 0 --output "$tgca_epoch_root/audit/residual_cwp.json"

    run_exact env CUDA_VISIBLE_DEVICES="$tgca_gpu" python tools/evaluate_mctformerplus_classification.py \
        --checkpoint "$tgca_checkpoint" --model mctformerplus \
        --class-token-init residual_cwp --voc-root "$tgca_voc_root" \
        --list-path "$tgca_val" --input-size 448 --batch-size 16 \
        --num-workers 8 --bootstrap-resamples 5000 \
        --bootstrap-seed 20270908 --output-dir "$tgca_eval_root/classification"
    ...
    run_exact env CUDA_VISIBLE_DEVICES="$tgca_gpu" python -m analysis.cwp_shared_presence \
        --output-dir "$tgca_epoch_root/shared_presence" \
        --variant-checkpoint "$tgca_checkpoint" --class-token-init residual_cwp \
    ...
    printf 'EPOCH_EXPERIMENT_COMPLETE epochs=%s finished=%s\n' \
        "$tgca_epoch_count" "$(date --iso-8601=seconds)" \
        | tee "$tgca_epoch_root/EXPERIMENT_COMPLETE"
done

run_exact python tools/summarize_mctformerplus_residual_cwp.py \
    --baseline-run-root "$tgca_baseline_root" --run-root "$tgca_run_root" \
