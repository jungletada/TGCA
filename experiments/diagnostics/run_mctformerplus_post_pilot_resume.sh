...
tgca_result_commit=22427d60bff5d1f6c4cc9c5c33f8912502d5a4b0
tgca_analysis_commit=596fbc6fb7d26771aa67fe6380681d2740411a71
tgca_post_log="$tgca_repo_root/results/queues/22427d6/mctformerplus-post-pilot.log"
exec > >(tee -a "$tgca_post_log") 2>&1

printf "POST_QUEUE_RESUME started=%s result_commit=%s analysis_commit=%s\n" \
    "$(date --iso-8601=seconds)" "$tgca_result_commit" "$tgca_analysis_commit"
if [[ "$(git rev-parse HEAD)" != "$tgca_analysis_commit" ]]; then
    echo "Analysis HEAD changed; refusing to run unreviewed diagnostics." >&2
    exit 2
fi
...
if ! git diff --quiet "$tgca_result_commit" "$tgca_analysis_commit" -- \
    models train_model_v2.py make_cam.py eval_cam_crf.py engine.py \
    datasets_cam.py utils.py experiments/ablations; then
    echo "Result-critical code differs between result and analysis commits." >&2
    exit 2
fi
