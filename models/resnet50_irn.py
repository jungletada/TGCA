...
class ResNet50IRN(nn.Module):
    def __init__(self):
        super(ResNet50IRN, self).__init__()
        # backbone
        ...
        self.mean_shift = ResNet50IRN.MeanShift(2)
    ...
    class MeanShift(nn.Module):
        def __init__(self, num_features):
            super(ResNet50IRN.MeanShift, self).__init__()
    ...
    def forward(self, x):
        x1 = self.stage1(x).detach()    # ds=4
        x2 = self.stage2(x1).detach()   # ds=4
        x3 = self.stage3(x2).detach()   # ds=8
        x4 = self.stage4(x3).detach()   # ds=16
        x5 = self.stage5(x4).detach()   # ds=16

        edge1 = self.fc_edge1(x1)   # ds=4
        edge2 = self.fc_edge2(x2)   # ds=4
        edge3 = self.fc_edge3(x3)[..., :edge2.size(2), :edge2.size(3)] # ds 8->4
        edge4 = self.fc_edge4(x4)[..., :edge2.size(2), :edge2.size(3)] # ds 16->4
        edge5 = self.fc_edge5(x5)[..., :edge2.size(2), :edge2.size(3)]  # ds 16->4
        
        edge_out = self.fc_edge6(torch.cat([edge1, edge2, edge3, edge4, edge5], dim=1)) # ds=4 dim=160

        dp1 = self.fc_dp1(x1)   # ds=4
        dp2 = self.fc_dp2(x2)   # ds=4
        dp3 = self.fc_dp3(x3)   # ds=8
        dp4 = self.fc_dp4(x4)[..., :dp3.size(2), :dp3.size(3)]  # ds 16->8
        dp5 = self.fc_dp5(x5)[..., :dp3.size(2), :dp3.size(3)]  # ds 16->8

        dp_up3 = self.fc_dp6(torch.cat([dp3, dp4, dp5], dim=1))[..., :dp2.size(2), :dp2.size(3)] # ds 8->4
        dp_out = self.fc_dp7(torch.cat([dp1, dp2, dp_up3], dim=1))  # ds=4
...
class AffinityDisplacementLoss(ResNet50IRN):
    ...
    def __init__(self, path_index):
        ...
        for i, pi in enumerate(path_index.path_indices):
            self.register_buffer(AffinityDisplacementLoss.path_indices_prefix + str(i), 
                                 torch.from_numpy(pi))

        self.register_buffer(
            'disp_target',
            torch.unsqueeze(
                torch.unsqueeze(torch.from_numpy(path_index.search_dst).transpose(1, 0), 0), -1).float())
...
class EdgeDisplacement(ResNet50IRN):
