...
class MCTViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=384, depth=12,
                 ...
                 drop_path_rate=0.1, norm_layer=partial(nn.LayerNorm, eps=1e-6), mask_type=None):
        super().__init__()
        ...
        self.dpr = [x.item() for x in torch.linspace(
        ...
        for i in range(depth):
            blocks.append(
                Block(dim=embed_dim,num_heads=num_heads,mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
                        qk_scale=qk_scale,drop=drop_rate,attn_drop=attn_drop_rate,drop_path=self.dpr[i],
