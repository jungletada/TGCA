...
def draw_heat_map(attn_maps, args, img_name):
    # Generate and save heatmaps using seaborn
    for layer in range(attn_maps.shape[0]):
        plt.figure(figsize=(20, 20))
        sns.heatmap(attn_maps[layer], cmap="coolwarm", cbar=True, square=True)
        plt.axis("off")
...
def _work(process_id, model, dataset, args):
    ...
    with torch.no_grad(), cuda.device(process_id):
        ...
        for iter_, pack in enumerate(tqdm(data_loader, position=process_id, desc=f'[PID{process_id}]')):
            ...
            # We first use only non-flip image for ablation
            attn_maps = outputs[0][:, 0, :, :].cpu().numpy()
            draw_heat_map(attn_maps, args, img_name)
