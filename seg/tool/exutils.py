...
def crop(img_temp, dim, new_p=True, hp=0, wp=0):
    h = img_temp.shape[0]
    w = img_temp.shape[1]
    trig_h = trig_w = False
    if (h > dim):
        if (new_p):
            hp = int(random.uniform(0, 1) * (h - dim))
        if len(img_temp.shape) == 2:
            img_temp = img_temp[hp:hp + dim, :]
        else:
            img_temp = img_temp[hp:hp + dim, :, :]
    elif (h < dim):
        trig_h = True
    if (w > dim):
        if (new_p):
            wp = int(random.uniform(0, 1) * (w - dim))
        if len(img_temp.shape) == 2:
            img_temp = img_temp[:, wp:wp + dim]
        else:
            img_temp = img_temp[:, wp:wp + dim, :]
    elif (w < dim):
        trig_w = True
...
def get_data_from_chunk(chunk, args):
    ...
    images = np.zeros((crop_size, crop_size, 3, len(chunk)))
    ori_images = np.zeros((crop_size, crop_size, 3, len(chunk)), dtype=np.uint8)
    gt_images = np.zeros((crop_size, crop_size, len(chunk)))
