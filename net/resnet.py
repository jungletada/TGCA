...
class ResNetV1c(ResNet):
    ...
    def __init__(self, num_classes, decay_parameter=0.996, **kwargs):
        super().__init__(deep_stem=True, avg_down=False, **kwargs)
        self.num_classes = num_classes
        self.head = nn.Conv2d(2048, num_classes, 1)
    ...
    def gwr_pooling_top_k(self, x, K=5):
        ...
        top_k_weights = torch.ones(K, device=x.device)  
        ...
        weights = all_weights.unsqueeze(0).unsqueeze(-1)  
        out = torch.sum(sorted_x * weights, dim=-2) / weights.sum()

        return out
    
    def forward(self, x):
        level_outputs = super().forward(x)
        final_x = level_outputs[-1]
        cls_output = self.head(final_x)
        cls_output = self.gwr_pooling_top_k(cls_output)
        return cls_output
