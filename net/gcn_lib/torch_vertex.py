...
class MRConv2d(nn.Module):
    ...
    def forward(self, x, edge_index, y=None):
        ...
        if y is not None:
            x_j = batched_index_select(y, edge_index[0]) # top-knn
        else:
            x_j = batched_index_select(x, edge_index[0]) # B x C x N x k
            
        x_j, _ = torch.max(x_j - x_i, -1, keepdim=True)  # B x C x N x 1
        b, c, n, _ = x.shape                             # B x C x N x 1
...
class DyGraphConv2d(GraphConv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=9, dilation=1, conv='edge', act='relu',
        ...
        self.dilated_knn_graph = \
            DenseDilatedKnnGraph(kernel_size, dilation, stochastic, epsilon)
    ...
    def forward(self, x, relative_pos=None):
        ...
        x = x.reshape(B, C, -1, 1).contiguous()                  # B x C x N x 1
        edge_index = self.dilated_knn_graph(x, y, relative_pos)  # get k-nearest neighbour and own
        x = super(DyGraphConv2d, self).forward(x, edge_index, y) # forward MRConv with x and edge
        return x.reshape(B, -1, H, W).contiguous() # B x C x H x W
...
class Grapher(nn.Module):
    ...
    """
    def __init__(self, in_channels, kernel_size=9, dilation=1, conv='mr', act='gelu', 
                 groups=6, norm='instance', bias=True, stochastic=False, epsilon=0.2, r=1, n=784, 
